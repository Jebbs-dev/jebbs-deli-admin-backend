...
class VendorRouter implements RouteController {
  ...
  public initialiseRoutes(): void {
    this.router.post(`${this.path}/register`, authenticated, upload("store").single("logo"), this.storeController.registerStore); //http://localhost:8080/api/Store/register
    ...
    this.router.get(`${this.path}/:id`, authenticated, this.storeController.fetchStoreByVendorId);
    this.router.patch(`${this.path}/:id`, authenticated, upload("Store").single("logo"), this.storeController.updateStore);
    this.router.delete(`${this.path}/:id`, authenticated, this.storeController.deleteStore);
  }
}
